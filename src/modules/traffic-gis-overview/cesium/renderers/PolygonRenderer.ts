...
export class PolygonRenderer {
  render(
    ...
    return features.map((f) => {
      ...
      const tag=featureLabel(f.name,color.toCssColorString(),layer.style.labelColor||'#ffffff',layer.style.labelSize||12);return viewer.entities.add({
        ...entityMeta(f),
        position: CoordinateAdapter.wgs84(center),
        billboard:layer.style.showLabel===false?undefined:{image:tag.image,width:tag.width,height:tag.height,verticalOrigin:Cesium.VerticalOrigin.BOTTOM,pixelOffset:new Cesium.Cartesian2(0,-4),disableDepthTestDistance:Number.POSITIVE_INFINITY,distanceDisplayCondition:new Cesium.DistanceDisplayCondition(0,16000)},
        polygon: {
          hierarchy: CoordinateAdapter.degreesArray(ring),
          material:new Cesium.StripeMaterialProperty({evenColor:color.withAlpha(layer.style.opacity??.22),oddColor:Cesium.Color.TRANSPARENT,repeat:18,offset:0,orientation:Cesium.StripeOrientation.VERTICAL}),
          outline: false,
          height: 40,
        },
        polyline: {
