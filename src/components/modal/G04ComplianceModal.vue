...
const data = ref<G04Data>({
  summary: { pendingTotal: 4, needSubmitThisMonth: 3, overdueCount: 1, involvedModules: 3, completenessRate: null },
  detailData: [],
  dataSource: '关键合规资料台账',
...
const summaryCards = computed(() => [
  ...
  { label: '本阶段待补齐', value: data.value.summary.completenessRate, unit: '', color: THEME_COLOR },
])
...
async function loadData() {
  ...
  data.value = {
    summary: {
      ...
      needSubmitThisMonth: getSummary('本月需提交') ?? 0,
      overdueCount: getSummary('逾期未提交') ?? 0,
      involvedModules: getSummary('涉及模块') ?? 0,
      completenessRate: getSummary('本阶段待补齐') ?? getSummary('资料完备率'),
    },
    detailData: rows,
    dataSource: resp.dataSource || '关键合规资料台账',
      ...
      <section class="g04-summary" aria-label="G04摘要">
        <div v-for="item in summaryCards" :key="item.label" class="summary-card">
          ...
          <div class="summary-value-row">
            <strong :style="{ color: item.color }">{{ item.value === null || item.value === undefined ? '--' : item.value }}</strong>
