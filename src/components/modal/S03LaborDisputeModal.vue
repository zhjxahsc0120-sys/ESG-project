...
const data = ref<S03Data>({
  summary: { pendingTotal: 4, newThisMonth: 2, closedThisMonth: 1, peopleCount: 18, amount: 68 },
  detailData: [],
  dataSource: '演示数据快照',
...
async function loadData() {
  ...
  data.value = {
    ...
    detailData: rows,
    dataSource: resp.isMock ? '演示数据快照' : (resp.dataSource || '劳务管理系统'),
              ...
              <table class="dispute-table">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="col-center">
                      <span class="status-tag" :style="{ color: getStatusColor(row.status), borderColor: getStatusColor(row.status) }">{{ row.status }}</span>
                    </td>
