...
const data = ref<G02Data>({
  summary: { expiringCount: 3, overdueCount: 2, expiringThisMonth: 4, involvedDepartments: 3, avgRemainingDays: 15 },
  detailData: [],
  dataSource: '演示数据快照',
...
const summaryCards = computed(() => [
  ...
  { label: '本月到期', value: data.value.summary.expiringThisMonth, unit: '项', color: THEME_COLOR },
...
async function loadData() {
  ...
  data.value = {
    summary: {
      ...
      expiringThisMonth: getSummary('本月到期'),
      involvedDepartments: getSummary('涉及部门'),
      avgRemainingDays: getSummary('平均剩余有效期'),
    },
    detailData: rows,
    dataSource: resp.isMock ? '演示数据快照' : (resp.dataSource || '证照管理系统'),
