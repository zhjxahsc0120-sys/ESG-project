...
const STATUS_COLORS = {
  normal: '#69e36f',
  processing: '#2f9cff',
  pending: '#ffb347', // 临期统一橙色
  danger: '#ff4f5e', // 逾期统一红色
  muted: '#8ba6c3',
} as const
...
const data = ref<G02Data>({
  summary: { expiringCount: 4, overdueCount: 1, expiringThisMonth: 4, involvedDepartments: 3, avgRemainingDays: 18 },
  detailData: [],
  dataSource: '证照许可台账',
...
const summaryCards = computed(() => [
  { label: '临期许可', value: data.value.summary.expiringCount, unit: '项', color: STATUS_COLORS.pending },
  { label: '逾期许可', value: data.value.summary.overdueCount, unit: '项', color: STATUS_COLORS.danger },
  { label: '30日内到期', value: data.value.summary.expiringThisMonth, unit: '项', color: THEME_COLOR },
  { label: '涉及部门', value: data.value.summary.involvedDepartments, unit: '个', color: THEME_COLOR },
  { label: '平均剩余有效期', value: data.value.summary.avgRemainingDays, unit: '天', color: THEME_COLOR },
])

// 到期分桶（保留人工校核通过的数据，不重新计算）
// 颜色规则：逾期红色、临期桶橙色、正常桶蓝色
const bucketData = computed(() => [
  { name: '已逾期', value: 1, color: STATUS_COLORS.danger },
  { name: '7日内', value: 1, color: STATUS_COLORS.pending },
  { name: '8-15日', value: 1, color: STATUS_COLORS.pending },
  { name: '16-30日', value: 2, color: STATUS_COLORS.pending },
  { name: '30日以上', value: 0, color: STATUS_COLORS.processing },
])

// 预警摘要（保留人工校核通过的数据，不重新计算）
...
async function loadData() {
  ...
  const rows: LicenseRow[] = list.map((item, index) => ({
    rowId: `G02-D-${index + 1}`,
  ...
  data.value = {
    summary: {
      expiringCount: getSummary('临期许可'),
      overdueCount: getSummary('逾期许可'),
      expiringThisMonth: getSummary('30日内到期') || getSummary('本月到期'),
      involvedDepartments: getSummary('涉及部门'),
      avgRemainingDays: getSummary('平均剩余有效期'),
    },
    detailData: rows,
    dataSource: resp.dataSource || '证照许可台账',
...
function initChart() {
  ...
  chart.setOption({
    ...
    grid: { left: 72, right: 50, top: 28, bottom: 24 },
    xAxis: {
      type: 'value',
      max: xAxisMax,
      minInterval: 1,
    ...
    >
      <header class="g02-header">
        <h2 id="g02-modal-title">
          ...
          <span class="title-name">当前临期及逾期许可事项数</span>
        </h2>
