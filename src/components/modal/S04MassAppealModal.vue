...
async function loadData() {
  ...
  data.value = {
    summary: {
      ...
      overdueCount: getSummary('逾期未办') || getSummary('已逾期'),
      avgProcessDays: getSummary('平均办理时长'),
    },
    detailData: rows,
    dataSource: resp.isMock ? '演示数据快照' : (resp.dataSource || '信访投诉系统'),
  ...
  <div class="s04-overlay" :class="{ acceptance: isAcceptanceMode }" @click="handleOverlayClick">
    ...
    >
      ...
      <footer class="s04-footer">
        <div class="footer-info" title="当前为演示数据快照，非正式数据库明细表">
          <FileCheck :size="13" />
        ...
        <div class="footer-info">
          <AlertCircle :size="13" />
          <span>逾期口径：来自接口状态字段，非办理期限计算</span>
        </div>
