...
// 基于开工日期和当前日期计算连续天数，与首页 S01 卡片口径一致
const continuousDays = computed(() => {
  ...
  const current = toUtcDate(data.value.currentDate)
...
const summaryCards = computed(() => [
  { label: '连续安全生产天数', value: String(continuousDays.value), unit: '天', color: STATUS_COLORS.normal },
...
async function loadData() {
  ...
  if (!resp) return
  data.value = {
    projectStartDate: resp.projectStartDate,
    currentDate: resp.currentDate,
    continuousDays: resp.continuousDays,
    currentStage: '路基桥涵施工',
    currentStageDetail: resp.currentStageDetail,
    countingStatus: resp.countingStatus as CountingStatus,
    latestInterruptDate: resp.latestInterruptDate,
    latestInterruptReason: resp.latestInterruptReason,
