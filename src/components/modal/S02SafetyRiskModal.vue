...
import { Check, Clock, FileCheck, ShieldAlert, X, AlertCircle, Flag } from 'lucide-vue-next'
...
// 当前选中行
const selectedRow = computed<RiskPointRow | null>(() => {
                  ...
                  >
                    ...
                    <td class="col-center">
                      <button type="button" class="row-action" @click.stop="toggleRow(row)">
                        {{ selectedRowId === row.rowId ? '取消关注' : '查看详情' }}
                      </button>
                    </td>
                  </tr>
                </tbody>
        ...
        <aside class="s02-side">
          ...
          <section class="panel reminder-panel">
            ...
            <div class="reminder-list">
              ...
              <div v-if="selectedRow" class="reminder-focus">
                <div class="reminder-focus-title">
                  <Flag :size="14" />
                  <span>当前关注</span>
                </div>
                <div class="reminder-focus-body">
                  <strong>{{ selectedRow.name }}</strong>
                  <span>等级：<em :class="levelClass(selectedRow.level)">{{ selectedRow.level }}</em></span>
                  <span>工点：{{ selectedRow.location }}</span>
                  <span>类型：{{ selectedRow.type }}</span>
                  <span>起始：{{ selectedRow.time }}</span>
                </div>
              </div>
