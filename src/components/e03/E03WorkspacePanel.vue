...
import { getE03Issues } from '@/services/api'
import type {
  E03CategoryFilter,
  E03IssueItem,
  E03IssuesPayload,
...
const emit = defineEmits<{
  ...
  selectIssue: [issue: E03IssueItem]
  clearSelection: []
  overviewReady: [issues: E03IssueItem[]]
}>()

const PAGE_SIZE = 3
const loading = ref(false)
const error = ref('')
const payload = ref<E03IssuesPayload | null>(null)
const page = ref(1)
...
function issueSection(issue: E03IssueItem) {
  return extractSectionLabel(issue.title, issue.locationText) || '未分区'
}

function issueTypeLine(issue: E03IssueItem) {
  return typeWithSection(issue.issueType || '水保问题', issueSection(issue))
}

function issueTitle(issue: E03IssueItem) {
  return stripSectionPrefix(issue.title) || issue.title || '—'
}

function issueLocation(issue: E03IssueItem) {
  return stripSectionPrefix(issue.locationText) || issue.locationText || '—'
}

const issues = computed(() => payload.value?.issues || [])

/** 一级筛选 key -> API statusGroup */
const STATUS_GROUP_BY_FILTER: Record<Exclude<E03CategoryFilter, 'ALL'>, string> = {
...
async function loadOverview() {
  ...
  error.value = ''
  try {
    const res = await getE03Issues()
    if (!res || res.code !== 0 || !res.data) {
      error.value = 'E03 数据暂不可用'
      payload.value = null
      emit('overviewReady', [])
      return
    }
...
function handleCategoryClick(key: E03CategoryFilter) {
...
function handleSelectIssue(issue: E03IssueItem) {
...
<template>
  <aside class="e03-panel">
    ...
    <template v-else>
      <section class="e03-stats" aria-label="状态分类统计">
        <button
      ...
      <div v-if="overview.overdueAmong > 0" class="e03-overdue-hint">
        其中逾期 <strong>{{ overview.overdueAmong }}</strong> 项
      </div>

      <div class="e03-list-title">当前未闭环事项</div>
      ...
      <div class="e03-list">
        ...
        >
          <div class="e03-row__top">
            <span class="e03-row__type">{{ issueTypeLine(issue) }}</span>
            <em class="e03-row__status" :class="{ overdue: issue.overdue }">{{ issue.status }}</em>
          </div>
          <div class="e03-row__title">{{ issueTitle(issue) }}</div>
          <div class="e03-row__loc">{{ issueLocation(issue) }}</div>
          <div class="e03-row__meta">
            <span>责任单位：{{ issue.responsibleOrgName || '—' }}</span>
            <span class="e03-row__deadline" :class="{ overdue: issue.overdue }">
              期限：{{ dateOnly(issue.deadline) }}
            </span>
          </div>
          <div v-if="!issue.canLocate" class="e03-row__foot">
            <span class="warn">无法定位</span>
          </div>
        </button>

        <p v-if="!pagedIssues.length" class="e03-empty">当前筛选暂无未闭环事项</p>
      </div>

      <nav class="e03-pager" aria-label="事项分页">
