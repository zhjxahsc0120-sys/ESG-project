...
import E02ClosureAnalysisPopover from '@/components/e02/E02ClosureAnalysisPopover.vue'
import E03ClosureAnalysisPopover from '@/components/e03/E03ClosureAnalysisPopover.vue'
...
import type { E02IssueItem } from '@/types/e02'
import type { E03IssueItem } from '@/types/e03'
...
const props = defineProps<{
  ...
  e02Active?: boolean
  e02Issues?: E02IssueItem[]
  e02SelectedIssueId?: number | null
  e03Active?: boolean
  e03Issues?: E03IssueItem[]
...
const featurePicker = ref<{ featureId: string; issues: E02IssueItem[] } | null>(null)
const e03FeaturePicker = ref<{ featureId: string; issues: E03IssueItem[] } | null>(null)
...
function issuesForFeature(featureId: string): E02IssueItem[] {
...
function e03IssuesForFeature(featureId: string): E03IssueItem[] {
...
function focusE02Issue(issue: E02IssueItem) {
...
function fitE02Issues(issues: E02IssueItem[]) {
  const featureIds = [
    ...new Set(
      issues
        .flatMap((i) => i.spatialLinks || [])
...
function focusE03Issue(issue: E03IssueItem) {
  const primaryLink = issue.spatialLinks?.find((sl) => sl.isPrimary) || issue.spatialLinks?.[0]
  if (!primaryLink?.featureId) return
  const isCorridor = /^section-\d/.test(primaryLink.featureId)
  void mapRef.value?.flyToFeature?.(primaryLink.featureId, {
...
function fitE03Issues(issues: E03IssueItem[]) {
  const featureIds = [
    ...new Set(
      issues
        .flatMap((i) => i.spatialLinks || [])
  ...
  <PanelCard flush>
    <div ref="panelRootRef" class="dashboard-gis-cesium-panel">
      ...
      <div ref="popoverAnchorRef" class="e02-popover-anchor">
        <E02ClosureAnalysisPopover
          v-if="e02Active"
          :issue-id="e02SelectedIssueId ?? null"
          :flip-left="popoverFlipLeft"
          @close="handleE02PopoverClose"
        />
        <E03ClosureAnalysisPopover
          v-if="e03Active"
          :issue-id="e03SelectedIssueId ?? null"
          :flip-left="popoverFlipLeft"
          @close="handleE03PopoverClose"
        />
      ...
      <div v-if="featurePicker" class="e02-feature-picker">
        ...
        >
          <span class="e02-feature-picker__status">{{ issue.status }}</span>
          <span class="e02-feature-picker__name">{{ issue.title }}</span>
        </button>
