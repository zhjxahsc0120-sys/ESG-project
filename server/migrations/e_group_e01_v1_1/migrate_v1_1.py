"""migrate_v1_1.py - E组公共闭环 & E01 V1.1 增量迁移执行器
========================================================
...
职责:
  ...
  - 已成功且校验和一致的版本跳过（幂等）
  - V1_1_000 门禁 GATE_FAIL 时停止后续迁移
  - V1_1_001 在首次执行前自动 bootstrap migration_history 表

用法:
  python -m server.migrations.e_group_e01_v1_1.migrate_v1_1
  或
  cd server && python -m migrations.e_group_e01_v1_1.migrate_v1_1
"""
...
def _bootstrap_migration_history_if_needed(conn) -> bool:
    """Check if esg_schema_migration_history exists; if not, execute V1_1_001 DDL.
    ...
    bootstrap_path = MIGRATION_DIR / "V1_1_001__migration_history_bootstrap.sql"
    ...
    print("[BOOTSTRAP] esg_schema_migration_history does not exist. "
...
def _extract_gate_result(conn, gate_path: Path) -> Optional[str]:
    """Execute the gate SQL and extract the gate_result from the last SELECT.
    ...
    try:
        ...
        if callable(nextset):
            while nextset():
                pass
        conn.commit()
        # After execution, query the last row's gate_result
        # The gate SQL ends with SELECT ... AS gate_result, ... AS gate_details
        # Use the session variable approach: execute a final query
        cursor.execute("SELECT @gate_failures")
        row = cursor.fetchone()
        failures = next(iter(row.values())) if isinstance(row, dict) else row[0]
        conn.rollback()  # Gate SQL uses SET session vars, no DDL to commit
        if failures == 0:
            return "GATE_PASS"
...
def run_migration(migration_dir: Optional[Path] = None, dry_run: bool = False,
    ...
    # --- Bootstrap phase: ensure migration history table exists ---
    _bootstrap_migration_history_if_needed(conn)

    # --- Gate phase: run V1_1_000 and check result ---
    gate_path = migration_dir / "V1_1_000__environment_gate.sql"
    if gate_path.is_file():
        ...
        if not gate_result.startswith("GATE_PASS"):
            ...
            _record_migration(
                conn, "V1_1_000", "environment gate",
    ...
    for file_path in sql_files:
        ...
        if version_key in ("V1_1_000", "V1_1_001"):
...
def main():
    ...
    print("=" * 60)
    print("E组 V1.1 Incremental Migration Executor")
